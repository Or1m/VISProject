...
namespace DataLayer.TableDataGateways
{
    ...
    {
        #region SQL Commands
        private static string SQL_INSERT                            = "INSERT INTO \"User\" (nick, gender, country, date_of_birth, registration_date, first_name, last_name, favorit_category_id) " +
                                                                      " VALUES (@nick, @gender, @country, @date_of_birth,@registration_date, @first_name, @last_name, @favorite_category_id)";

        private static string SQL_UPDATE                            = "UPDATE \"User\" SET nick=@nick, gender=@gender," +
                                                                      "country=@country, date_of_birth=@date_of_birth, registration_date=@registration_date," +
                                                                      "first_name=@first_name, last_name=@last_name, favorite_category_id=@favorite_category_id WHERE user_id=@user_id";

        private static string SQL_DELETE_BY_ID                      = "UPDATE \"User\" SET deleted=1 where user_id=@user_id";

        private static string SQL_DELETE_NICK                       = "UPDATE \"User\" SET deleted=1 where nick=@nick";

        private static string SQL_SELECT_ALL_USERS_HEADER           = "SELECT user_id, nick, country FROM \"User\"";

        private static string SQL_SELECT_USER_BY_ID                 = "SELECT user_id, nick, gender, country, date_of_birth, registration_date, first_name, last_name, favorite_category_id, deleted FROM \"User\" WHERE user_id=@user_id";

        private static string SQL_SELECT_USER_BY_NICK               = "SELECT user_id, nick, gender, country, date_of_birth, registration_date, first_name, last_name, favorite_category_id, deleted FROM \"User\" WHERE nick=@nick";
